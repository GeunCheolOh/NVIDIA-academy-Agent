...
JSON만 출력하세요."""
        ...
        try:
            # YOUR CODE HERE - LLM 호출 및 JSON 파싱
            ...
            # 
            # LLM에게 라우팅 결정을 요청하고 JSON 응답을 파싱
            route = "direct"  # 위의 YOUR CODE HERE를 채우면 이 줄을 삭제하세요
            reasoning = "기본 경로"  # 위의 YOUR CODE HERE를 채우면 이 줄을 삭제하세요
    ...
    def _vectordb_node(self, state: AgentState) -> dict:
        ...
        try:
            ...
            # YOUR CODE HERE - retriever.invoke()로 문서 검색
            # docs = self.d2l_retriever.invoke(question)
            # 
            # D2L 교재 벡터 스토어에서 관련 문서 검색
    ...
    def _websearch_node(self, state: AgentState) -> dict:
        ...
        try:
            ...
            # YOUR CODE HERE - tavily_tool.invoke()로 웹 검색
            # search_results = self.tavily_tool.invoke(question)
            # 
            # Tavily API를 사용한 웹 검색
            search_results = []  # 위의 YOUR CODE HERE를 채우면 이 줄을 삭제하세요
    ...
    def _direct_llm_node(self, state: AgentState) -> dict:
        ...
        try:
            ...
            # YOUR CODE HERE - LLM에 직접 질문
            # conversation = messages + [HumanMessage(content=question)]
            # response = self.llm.invoke(conversation)
            # 
            # 대화 이력을 포함하여 LLM에 직접 질문
            response = None  # 위의 YOUR CODE HERE를 채우면 이 줄을 삭제하세요
...
참고 자료에서 답을 찾을 수 없다면 솔직하게 말씀해주세요."""
    ...
    def _route_question(self, state: AgentState) -> str:
        ...
        Returns:
            다음 노드 이름 ("vectordb", "websearch", "direct")
        """
        # YOUR CODE HERE - state["route"] 반환
        # return state["route"]
        # 
        # router_node에서 결정한 경로를 반환
        # 이 값이 조건부 엣지의 키로 사용됨
